...
// returns true or false if the s match allowed HTML Name pattern. https://stackoverflow.com/questions/925994/what-characters-are-allowed-in-an-html-attribute-name.
// usefull to check attribute or token name.
//
//	returns FALSE if s is empty
//
// must be trim before, if required
func IsValidHTMLName(s string) (_ret bool) {
	if s == "" {
		return false
	}
