...
type SectionIckNavbar struct{ SectionIcecakeDoc }

func (cmp *SectionIckNavbar) RenderContent(out io.Writer) error {
	html.WriteString(out, `<h2>Icecake Bulma Navbar</h2>`)
	html.WriteString(out, `<p>ick.Navbar is an icecake snippet providing the HTML rendering for a `, linkBulmaNavbar, `, with extra features and usefull go wasm APIs.</p>`)
	...
	html.WriteString(out, `<h3>ick.Navbar APIs</h3>`)
