...
func (cmp *SectionBulmaButton) RenderContent(out io.Writer) error {
	...
	uA1 := bulma.Button(*html.ToHTML("Click Link"), "", "#")
	uA2 := bulma.Button(*html.ToHTML("Trigger Event"), "uA2", "")
	...
	html.WriteString(out, `<div class="box spaceout mr-5">`)
	uB0 := bulma.Button(*html.ToHTML("Default"), "", "")
	uB1 := bulma.Button(*html.ToHTML("Primary color"), "", "").SetColor(bulma.COLOR_PRIMARY)
	uB2 := bulma.Button(*html.ToHTML("Light color"), "", "").SetColor(bulma.COLOR_PRIMARY).SetLight(true)
	uB3 := bulma.Button(*html.ToHTML("Link color"), "", "").SetColor(bulma.COLOR_LINK)
	uB4 := bulma.Button(*html.ToHTML("Outlined"), "", "").SetColor(bulma.COLOR_PRIMARY).SetOutlined(true)
	uB5 := bulma.Button(*html.ToHTML("Rounded"), "", "").SetColor(bulma.COLOR_PRIMARY).SetRounded(true)
	...
	html.WriteString(out, `<div class="box spaceout mr-5">`)
	uC1 := bulma.Button(*html.ToHTML("Standard"), "", "").SetColor(bulma.COLOR_PRIMARY)
	uC2 := bulma.Button(*html.ToHTML("Loading"), "", "").SetColor(bulma.COLOR_PRIMARY).SetLoading(true)
	uC3 := bulma.Button(*html.ToHTML("Disabled"), "", "").SetColor(bulma.COLOR_PRIMARY).SetDisabled(true)
