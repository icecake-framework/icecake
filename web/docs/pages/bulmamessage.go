...
type SectionIckMessage struct{ SectionIcecakeDoc }

func (sec *SectionIckMessage) RenderContent(out io.Writer) error {
	html.WriteString(out, `<h2>Message</h2>`+
		`<p>ick.Message is an icecake snippet providing the HTML rendering for a `, linkBulmaMessage, ` with extra features and usefull Go APIs.</p>`)
