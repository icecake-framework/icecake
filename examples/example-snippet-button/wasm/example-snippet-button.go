...
func main() {
	...
	b0 := ui.NewButton(html.String("Click here"))
	...
	b1 := ui.NewButton(html.String("Toggle Rounded"))
	b1.Tag().Attributes().AddClasses("m-2 is-link").SetAttribute("data-example", "1")
	content.InsertSnippet(dom.INSERT_LAST_CHILD, b1, nil)
	b1.DOM.AddMouseEvent(event.MOUSE_ONCLICK, OnClickExample)
	btn = append(btn, b1)

	b2 := ui.NewButton(html.String("Toggle Outlined"))
	b2.Tag().Attributes().AddClasses("m-2 is-link").SetAttribute("data-example", "2")
	content.InsertSnippet(dom.INSERT_LAST_CHILD, b2, nil)
	b2.DOM.AddMouseEvent(event.MOUSE_ONCLICK, OnClickExample)
	btn = append(btn, b2)

	b3 := ui.NewButton(html.String("Toggle Loading"))
	b3.Tag().Attributes().AddClasses("m-2 is-link").SetAttribute("data-example", "3")
	content.InsertSnippet(dom.INSERT_LAST_CHILD, b3, nil)
	b3.DOM.AddMouseEvent(event.MOUSE_ONCLICK, OnClickExample)
	btn = append(btn, b3)

	b4 := ui.NewButton(html.String("Toggle Disabled"))
	b4.Tag().Attributes().AddClasses("m-2 is-link").SetAttribute("data-example", "4")
	content.InsertSnippet(dom.INSERT_LAST_CHILD, b4, nil)
	b4.DOM.AddMouseEvent(event.MOUSE_ONCLICK, OnClickExample)
	btn = append(btn, b4)

	b5 := ui.NewButtonLink(html.String("Go To Home"), "/")
	b5.SetOutlined(true)
	b5.Tag().Attributes().AddClasses("m-2 is-info").SetAttribute("data-example", "5")
