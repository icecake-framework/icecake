...
func main() {
	...
	html1 := html.String(`<ick-button Id="btne1" class="m-2 is-primary is-light" data-example=6 Title="Click Embedded" IsOutlined/>`)
	content.InsertHTML(dom.INSERT_LAST_CHILD, html1, nil)
	dom.Id("btne1").AddMouseEvent(event.MOUSE_ONCLICK, OnClick)

	html2 := html.String(`<ick-button class="m-2 is-primary is-light" data-example=7 Title="Embedded URL" IsOutlined HRef='/'/>`)
