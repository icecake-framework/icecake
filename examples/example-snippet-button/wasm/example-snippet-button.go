...
func main() {
	...
	b0.Tag().Attributes().AddClasses("m-2 is-link")
	...
	b1.Tag().Attributes().AddClasses("m-2 is-link").SetAttribute("data-example", "1", true)
	...
	b2.Tag().Attributes().AddClasses("m-2 is-link").SetAttribute("data-example", "2", true)
	...
	b3.Tag().Attributes().AddClasses("m-2 is-link").SetAttribute("data-example", "3", true)
	...
	b4.Tag().Attributes().AddClasses("m-2 is-link").SetAttribute("data-example", "4", true)
	...
	b5.SetOutlined(true)
	b5.Tag().Attributes().AddClasses("m-2 is-info").SetAttribute("data-example", "5", true)
	content.InsertSnippet(dom.INSERT_LAST_CHILD, b5, nil)

	html1 := html.HTMLString(`<ick-button Id="btne1" class="m-2 is-primary is-light" data-example=6 Title="Embedded with event" IsOutlined/>`)
	content.InsertHTML(dom.INSERT_LAST_CHILD, html1, nil)
	dom.Id("btne1").AddMouseEvent(event.MOUSE_ONCLICK, OnClick)

	html2 := html.HTMLString(`<ick-button class="m-2 is-primary is-light" data-example=7 Title="Embedded with URL" IsOutlined HRef='/'/>`)
